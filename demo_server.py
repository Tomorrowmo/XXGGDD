...
from src.QJZ_fluent_post.http_api import create_fluent_router
...
async def chart_pressure_curve(body: dict):
    ...
    from src.plotter import build_pressure_curve
...
async def llm_diagnose(body: dict):
    ...
    from src.plotter import compute_channel_stats
    from src.llm_client import SYSTEM_PROMPT, _build_validation_summary
    from src.rag_client import chat_stream
...
async def chart_channels(filename: str):
    ...
    from src.plotter import _read_file, extract_channels
...
async def vlm_chat(body: dict):
    ...
    from src.vlm_client import chat_stream as vlm_chat_stream
    from src.rag_client import get_model_config
...
async def vlm_analyze_blow(body: dict):
    ...
    from src.vlm_client import analyze_image_stream
    from src.rag_client import get_model_config
...
async def ragflow_models():
    """获取所有可用模型列表"""
    from src.rag_client import load_models
...
async def fluent_vlm_analyze(body: dict):
    ...
    from src.vlm_client import analyze_cfd_image_stream
    from src.rag_client import get_model_config
...
if __name__ == "__main__":
    uvicorn.run("demo_server:app", host="0.0.0.0", port=8501, reload=True)
