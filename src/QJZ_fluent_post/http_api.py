...
project_root 为 FluentAnalysis 项目根（含 src.QJZ_fluent_post 的目录）。
"""
...
def create_fluent_router(project_root: Path) -> APIRouter:
    ...
    def _resolve_case_output_dir(cas_p: Path, case_name: str | None = None) -> Path:
        from src.QJZ_fluent_post.output_cache import resolve_case_output_dir
    ...
    def _symmetry_response(out_dir: Path, section_keys: list[str] | None = None) -> dict:
        from src.QJZ_fluent_post.output_cache import load_symmetry_sections
    ...
    async def fluent_output_load(cas_path: str, case_name: str = ""):
        ...
        from src.QJZ_fluent_post.output_cache import json_sanitize, load_cached_output
    ...
    async def fluent_output_status(cas_path: str, case_name: str = ""):
        ...
        if not cas_p.is_file():
            return JSONResponse({"error": f"找不到 cas 文件: {cas_p}"}, status_code=404)
        out_dir = _resolve_case_output_dir(cas_p, _case_name_from(query=case_name))
        from src.QJZ_fluent_post.output_cache import output_availability
        return JSONResponse({
    ...
    async def fluent_output_clear(body: dict):
        ...
        from src.QJZ_fluent_post.output_cache import clear_output_dir
    ...
    async def fluent_zones(cas_path: str):
        ...
        if not cas_p.is_file():
            return JSONResponse({"error": f"找不到 cas 文件: {cas_p}"}, status_code=404)
        try:
            from src.QJZ_fluent_post.zones import parse_zones
    ...
    async def fluent_quick_load(body: dict):
        ...
        if err:
            return err
        from src.QJZ_fluent_post.output_cache import json_sanitize, parse_force_flag, parse_inlet_report_txt, parse_wall_forces_txt
        force = parse_force_flag(body.get("force", False))
        out_dir = _resolve_case_output_dir(cas_p, _case_name_from(body))
        from src.QJZ_fluent_post.zones import parse_zones
        from src.QJZ_fluent_post.wall_forces import (
            compute_wall_forces,
            save_wall_forces_report_txt,
            wall_forces_to_serializable,
        )
        from src.QJZ_fluent_post.inlet_conditions import compute_inlet_report, save_inlet_report_txt
    ...
    async def fluent_inlet_only(body: dict):
        ...
        if err:
            return err
        from src.QJZ_fluent_post.output_cache import json_sanitize, parse_force_flag, parse_inlet_report_txt
        force = parse_force_flag(body.get("force", False))
        out_dir = _resolve_case_output_dir(cas_p, _case_name_from(body))
        from src.QJZ_fluent_post.inlet_conditions import compute_inlet_report, save_inlet_report_txt
        try:
    ...
    async def fluent_wall_only(body: dict):
        ...
        if err:
            return err
        from src.QJZ_fluent_post.output_cache import json_sanitize, parse_force_flag, parse_wall_forces_txt
        force = parse_force_flag(body.get("force", False))
        out_dir = _resolve_case_output_dir(cas_p, _case_name_from(body))
        from src.QJZ_fluent_post.wall_forces import (
            compute_wall_forces,
    ...
    async def fluent_symmetry(body: dict):
        ...
        from src.QJZ_fluent_post.output_cache import parse_force_flag, section_pngs_complete
        force = parse_force_flag(body.get("force", False))
        out_dir = _resolve_case_output_dir(cas_p, _case_name_from(body))
        from_cache = False
        try:
            if not force and section_pngs_complete(out_dir, section):
                ...
                from src.QJZ_fluent_post.symmetry_plot import plot_symmetry
    ...
    async def fluent_x_slice_run(body: dict):
        ...
        if err:
            return err
        from src.QJZ_fluent_post.output_cache import (
            XSLICE_CSV,
            XSLICE_META,
        ...
        out_dir = _resolve_case_output_dir(cas_p, _case_name_from(body))
        from src.QJZ_fluent_post.x_slice_average import plot_x_slice_field, run_x_slice_analysis, x_slice_plot_filename
    ...
    async def fluent_x_slice_plot(body: dict):
        ...
        try:
            from src.QJZ_fluent_post.x_slice_average import plot_x_slice_field, x_slice_plot_filename
